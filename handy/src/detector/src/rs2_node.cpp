...
class RS2Node : public rclcpp::Node
{
  public:
    ...
    : Node("rs2_node")
    {
      cfg_.enable_stream(RS2_STREAM_COLOR, 640, 480, RS2_FORMAT_BGR8);
      pipe_.start(cfg_);
      ...
      timer_ = this->create_wall_timer(50ms, std::bind(&RS2Node::timer_callback, this));
    }
  ...
  private:
    void timer_callback()
    {
      ...
      cv::Mat img(cv::Size(640, 480), CV_8UC3, (void*)color.get_data());
