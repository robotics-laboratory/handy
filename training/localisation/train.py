...
def normal_dist_target(center, rad, width, height, sigma, device="cpu"):
    ...
    if (w > center[0] > 0) and (h > center[1] > 0):
        ...
        target_ball_position[target_ball_position < 0.05] = 0.0
...
class Ball_Detection_Loss(nn.Module):
    ...
    def forward(self, pred_ball_position, target_ball_position):
        ...
        loss_ball_x = -torch.mean(
            x_target * torch.log(x_pred + self.epsilon)
            + (1 - x_target) * torch.log(1 - x_pred + self.epsilon)
        )
        loss_ball_y = -torch.mean(
            y_target * torch.log(y_pred + self.epsilon)
...
class LitLocalisation(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for sample_index in range(pred.size(0)):
            target[sample_index] = normal_dist_target(
                data[sample_index]["ball_center"],
                data[sample_index]["ball_rad"],
                self.model.width,
                self.model.height,
                self.sigma,
                self.device,
            ...
            ball_coords[sample_index] = torch.tensor(
                data[sample_index]["ball_center"], device=self.device
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--width", type=int, default=320, help="Width of the input image"
    )
    parser.add_argument(
        "--height", type=int, default=192, help="Height of the input image"
    )
    parser.add_argument(
        "--n_last", type=int, default=5, help="Number of last frames to consider"
    )
    parser.add_argument("--batch_size", type=int, default=8, help="Batch size")
    parser.add_argument(
        "--dropout_p", type=float, default=0.5, help="Dropout probability"
    )
    parser.add_argument(
        "--sigma", type=float, default=None, help="Sigma for the Gaussian distribution"
    )
    args = parser.parse_args()

    model = BallLocalisation(dropout_p=args.dropout_p, n_last=args.n_last)
    data_module = LocalisationDataModule(
        args.train_dir,
        args.val_dir,
        args.width,
        args.height,
