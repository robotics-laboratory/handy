...
class LitDetector(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_loss", loss, on_step=True, on_epoch=True, batch_size=len(images)
        )
        return loss
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(
            self.parameters(), lr=0.0005, momentum=0.9, nesterov=True
        )
...
if __name__ == "__main__":
    ...
    dm = DetectionDataModule(
        args.data_dir, args.annot_file, args.width, args.height, args.batch_size
    )
