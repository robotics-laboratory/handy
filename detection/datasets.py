...
def generate_bounding_boxes(directory_path, output_path):
    """
    Generate bounding boxes for the masks
    in the given directory and save them to the output file.
...
class DetectionDataset(Dataset):
    ...
    Args:
        ...
        transforms (callable, optional): Optional transforms
                                            to be applied to the images.
        train (bool): Whether to use the training or testing set.
    """

    def __init__(
        self, image_dir, annot_file, width, height, transforms=None, train=True
    ):
...
class DetectionDataModule(L.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        self.valid_dataset = DetectionDataset(
            self.data_dir,
            self.annot_dir,
            self.width,
            self.height,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_dir",
        type=str,
        default="datasets/annotated_12_02_23/ball",
        help="Path to the directory containing the images",
    )
    parser.add_argument(
        "--annot_file",
        type=str,
        default="datasets/annotated_12_02_23/bounding_boxes.json",
        help="Path to the file containing the bounding box annotations",
    )
    parser.add_argument(
        "--width",
        type=int,
        default=300,
        help="The width to which the images will be resized",
    )
    parser.add_argument(
        "--height",
        type=int,
        default=300,
        help="The height to which the images will be resized",
    )

    args = parser.parse_args()

    dataset = DetectionDataset(
        args.image_dir, args.annot_file, args.width, args.height, train=True
    )
